...
class TestBaseLoggerInit:
    ...
    def test_level_set_correctly(self) -> None:
        logger = BaseLogger(name="test_base_logger_level", level="warning")
    ...
    def test_default_handler_added_only_once(self) -> None:
        ...
        logger = logging.getLogger(name)
        named_handlers = [
            h for h in logger.handlers if h.get_name() == f"{name}_default_handler"
        ]
...
class TestWrapPythonLogger:
    ...
    def test_no_handlers_added_on_wrap(self) -> None:
        ...
        BaseLogger.wrap_python_logger(existing)
